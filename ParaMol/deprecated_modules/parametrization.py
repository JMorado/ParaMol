...
class Parametrization(Task):
    ...
    def run_task(self, settings, systems, parameter_space=None, objective_function=None, optimizer=None, adaptive_parametrization=False):
        ...
        if optimizer is None:
            optimizer = self.create_optimizer(settings.optimizer)
        else:
        ...
        for system in systems:
            ...
            # Calculate prior widths and apply jacobi preconditioning
        ...
        # TODO: assess if is really necessary to apply charge correction AFTER the parametrization
        """
        # Update the parameters in the force field
        for system in systems:
            # Print Initial Info of Objective Function
            objective_function.f(parameter_space.optimizable_parameters_values_scaled, opt_mode=False)
            system.force_field.optimizable_parameters_values = parameters_values * parameter_space.prior_widths
            system.force_field.update_force_field(parameter_space.optimizable_parameters_values)
    ...
    def _perform_assertions(self, settings, system):
        ...
        if settings.objective_function_settings["energies"]:
            assert system.ref_energies is not None, "Energies were not set."
        if settings.objective_function_settings["forces"]:
            assert system.ref_forces is not None, "Forces were not set."
        if settings.objective_function_settings["esp"]:
    ...
    def _perform_optimization(self, settings, optimizer, objective_function, parameter_space):
        ...
        if constrained:
            ...
            constraints = self._get_constraints(scipy_method=settings.optimizer["scipy_method"],
    ...
    def _apply_charge_correction(self, system):
        ...
        if "NonbondedForce" in system.force_field.force_field:
            ...
            print("Total charge before correction: {}e .".format(total_charge))
            charge_correction = total_charge / system.n_atoms
            print("Charge correction {}e per atom.".format(charge_correction))
            ...
            total_charge = self._get_total_charge(system)
            print("Total charge after correction: {}e .".format(total_charge))

            return total_charge

        else:
            print("Not applying charge correction.")
            return 1
    # -----------------------------------------------------------#
    #                                                            #
    #                       STATIC METHODS                       #
